...
class Coupon extends connect
{
    ...
    public function addCoupon($name,$start_date,$end_date,$quantity,$status,$coupon_code,$type,$coupon_value) {
        $sql = 'INSERT INTO coupons(name, start_date, end_date, quantity, status, coupon_code, type, coupon_value, created_at, updated_at)
                 VALUES (?,?,?,?,?,?,?,?,now(),now())';
        $stmt = $this->connect()->prepare($sql);
    ...
    public function updateCoupon($name,$start_date,$end_date,$quantity,$status,$coupon_code,$type,$coupon_value) {
        $sql = 'UPDATE coupons SET name=?, start_date=?, end_date=?,  quantity=?, status=?, coupon_code=?, type=?,
                                     coupon_value=?, updated_at=now() WHERE coupon_id= ? ';
        $stmt = $this->connect()->prepare($sql);
